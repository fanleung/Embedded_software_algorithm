...
void MERGESORT()
{
	...
	MERGEPASS1(SR,TR);				//准备工作，一边扫描，一边归并，生成初始化队列
	disp(TR);						//显示这一轮归并操作后的结果
	while(1)						//在两个数组之间反复进行归并操作
	{
		if(MERGEPASS(TR,SR)==1)	//再进行一轮归并，结果返回到SR[]
			return ;			//如果本轮归并只进行一次，则结束排序
		disp(SR);				//否则显示此轮结果并进行下一轮归并
		MERGEPASS(SR,TR);		//进行一轮归并，结果在TR[]
		disp(TR);				//显示这一轮归并操作后的结果	
	}
}
